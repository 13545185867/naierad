...
public class WeixinAccountTokenTask{
		...
		for(JwWebJwid account : list){
			try {
				restWeiXinToken(account);
			 	...
			 	LogUtil.info("---------定时任务【重置超过2小时失效token】失败公众号------------------"+account.getName());
			}
		}
	...
	public void resetToken(String jwid) {
		long start = System.currentTimeMillis();
		
		List<JwWebJwid> list =null;
		//= systemService.findHql(hql);
		list = jwWebJwidServicenew.getByJWid(jwid);
		  LogUtil.info("===================定时任务【重置超过2小时失效token】开始===================");
		for(JwWebJwid account : list){
			try {
				restWeiXinToken(account);
		...
		if (null != jsonObject) {
			try {
				...
				account.setAddtoekntime(getAccessTokenDate);
				 ...
				 LogUtil.info("---------定时任务定时任务【重置超过2小时失效token】成功公众号------------------" + account.getName());
			} catch (Exception e) {
				// 获取token失败
				...
				//account.setAddtoekntime(getAccessTokenDate);
				// 重置获取token时间【上次定时任务获取Token时间-失败保存】
				account.setAddtoekntime(account.getAddtoekntime());
				jwWebJwidServicenew.doEdit(account);
				 LogUtil.info("---------定时任务定时任务【重置超过2小时失效token】失败保存公众号------------------" + account.getName());
			}
		}
	}
}